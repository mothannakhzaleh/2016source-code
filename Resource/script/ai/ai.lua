...
function ai_idle(c)
	...
	if ai_type == 32 then				
		local mxhp = Mxhp(c)
		local _mxhp=math.floor(mxhp)
		SetChaAttr(c,ATTR_HP,_mxhp)	
		SyncChar(c, ATTR_HP)
	end
...
function special_ai(c, t,ai_type)
	...
	if ai_type == HLNFLA then
		...
		local StateLv = GetChaStateLv ( c , STATE_TJ )
		if StateLv~=0 then
			State_Tj_Rem ( c , StateLv )
		end
