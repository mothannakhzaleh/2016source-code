...
UIRender::UIRender() :
	_nGuiWidth(1024), _nGuiHeight(768), _fScreenScaleX(1.0), _fScreenScaleY(1.0), _fDrawScaleX(1.0), _fDrawScaleY(1.0), _p2DSprite(NULL), _nTex(-1)
{
  _rtFrame.left	  = 0;
  _rtFrame.top	  = 0;
  _rtFrame.right  = 1;
...
void UIRender::RenderTextureRect(int nX, int nY, LETexRect* pRect, float rotation)
{
  ...
  RenderSprite(pTexture, &TexRect, &vecScale, &vecDest, pRect->dwColor, rotation);
}
...
void CGuiPic::RenderAll(int x, int y, BYTE bAlpha)
{
  for(unsigned int i = 0; i < _max; i++)
  {
	GetRender().RenderTextureRect(x, y, &_pImage[i], (DWORD)(bAlpha << 24) | 0x00FFFFFF);
  }
}
