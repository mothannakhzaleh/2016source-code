...
void CSMallMap2D::InitScene()
{
  ...
  mi.subset_num	   = 1;
  mi.vertex_num	   = 4;
  mi.vertex_seq	   = LE_NEW(MPVector3[mi.vertex_num]);
  mi.blend_seq	   = LE_NEW(drBlendInfo[mi.vertex_num]);
  mi.vercol_seq	   = LE_NEW(DWORD[mi.vertex_num]);
  mi.texcoord0_seq = LE_NEW(MPVector2[mi.vertex_num]);
  mi.texcoord1_seq = LE_NEW(MPVector2[mi.vertex_num]);
...
void CSMallMap2D::RenderScene()
{
  ...
  for(m = 0; m < 3; m++)
  {
	for(int n = 0; n < 3; n++)
	{
	  ...
	  {
		g_Render.SetVertexShaderConstant(9, &D3DXVECTOR4(float(sx), float(sy), 0, 1), 1);
		g_Render.SetVertexShaderConstant(10, &D3DXVECTOR4(float(sx) + SHOWRSIZE, float(sy), 0, 1), 1);

		g_Render.SetVertexShaderConstant(11, &D3DXVECTOR4(float(sx) + SHOWRSIZE, float(sy) + SHOWRSIZE, 0, 1), 1);
		g_Render.SetVertexShaderConstant(12, &D3DXVECTOR4(float(sx), float(sy) + SHOWRSIZE, 0, 1), 1);
	  }
#ifdef MGR
	  _pVB->DrawSubset(0);
#else
  ...
  for(m = 0; m < _TexHnum; m++)
  {
	for(int n = 0; n < _TexWnum; n++)
	{
	  ...
	  if(_pTex[idx] == _pTexMask)
		continue;
	  if(vecColor[idx + 1] == 0 && vecColor[idx + 1 + 1] == 0 && vecColor[idx + _TexWnum + 1 + 1] == 0 && vecColor[idx + _TexWnum + 1] == 0)
		continue;
