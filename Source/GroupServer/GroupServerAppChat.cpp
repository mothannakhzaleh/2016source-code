...
void GroupServerApp::Execute(Player *ply, std::string msg[])
{
	if(msg[1].compare("sayall") == 0) // 公告
	{
		...
		l_wpk.WriteString(msg[2].c_str());								//说话的内容
	...
	else if(msg[1].compare("say") == 0)
	{
		...
		if(l_dst && l_dst->m_currcha >= 0)
		{
			...
			l_wpk.WriteString("System");		//说话人
			l_wpk.WriteString(l_dst->m_chaname[l_dst->m_currcha].c_str());	//目标
			l_wpk.WriteString(msg[3].c_str());									//说话内容
			Player *l_dst1[2];
			l_dst1[0]	=l_dst;
...
void GroupServerApp::CP_SAY2YOU(Player *ply,DataSocket *datasock,RPacket &pk)
{
	...
	}else
	{
		...
		}else
		{
			...
			l_wpk.WriteString(l_dst->m_chaname[l_dst->m_currcha].c_str());	//目标
			l_wpk.WriteString(l_content);									//说话内容
			Player *l_dst1[2];
			l_dst1[0]	=ply;
			l_dst1[1]	=l_dst;
